...
class Classification:
    ...
    def LinearSVM(self):    
        ...
        # with open('LSVM.csv','a') as f:
        #     np.savetxt(f, np.array([meanLSVM_Pd, maenLSVM_Pfa]).reshape(1,-1), delimiter=',')
        
        # for i in range(len(Pfa_target)):
        #     if (y_pred2[i] and self.y_test[i]) == 1:
        #         count_EH += 1
        #     else:
        #         count_No_EH += 1
    ...
    def S1(self):
        ...
        for i in range(len(Pfa_target)):
            ...
            if (y_pred[i] and self.y_test[i]) == 1:
                count_EH += 1
            else:
                count_No_EH += 1
